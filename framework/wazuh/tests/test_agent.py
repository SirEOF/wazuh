...
@patch('wazuh.common.client_keys', new=os.path.join(test_data_path, 'client.keys'))
@patch('wazuh.core.core_agent.chown')
@patch('wazuh.core.core_agent.chmod')
...
def test_agent_get_agent_groups(test_data, group_list, expected_result):
    def _get_hash_mock(*args, **kwargs):
        if 'merged.mg' in args[0]:
            return get_hash(os.path.join(test_data_path, 'merged.mg'), *args[1:], **kwargs)
        elif 'agent.conf' in args[0]:
            return get_hash(os.path.join(test_data_path, 'agent.conf'), *args[1:], **kwargs)
        else:
            return get_hash(*args, **kwargs)

    with patch('sqlite3.connect', return_value=test_data.global_db):
        with patch('wazuh.agent.get_hash') as get_hash_mock:
            get_hash_mock.side_effect = _get_hash_mock
